...
if ($_SERVER['REQUEST_METHOD'] == "POST" && $data) {
    ...
    $dateTime       = isset($data->dateTime)         ? htmlentities($data->dateTime) : null;
    ...
    if($action=="insert"){
        ...
        $data['datetime_schedule'] = $dateTime;
 
        $obj->update('applications', $data, " application_id = '{$application_id}' ");

        $message = "Your request has been approved. Your schedule will be on: {$dateTime}";

        sendEmail($email, "approved ", $message);
    }
